...
export async function POST(req) {
  try {
    ...
    // Summarize test results instead of passing all student scores
    const summarizedTests = formattedTests.map(t => {
      const scores = t.scores.map(s => parseFloat(s.percentage)).filter(p => !isNaN(p));
    ...
    const snapshot = {
      ...
      todays_attendance: {
        ...
        // Omitted individual lists to save tokens
      },
      recent_tests: summarizedTests
    };
...
6. Use clear, professional formatting (headers, bold text, bullet points). Format currency as ₹XX,XXX.`;
    ...
    return NextResponse.json({
      answer: chatCompletion.choices[0]?.message?.content || "No response generated by AI.",
      usage: {
